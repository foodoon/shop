...
/*     */ 
/* 120 */     model.addAttribute("userDegreeList", userDegreeList);
/* 121 */     model.addAttribute("familyMembersList", familyMembersList);
/* 122 */     model.addAttribute("incomeDescList", incomeDescList);
/* 123 */     model.addAttribute("workSeniorityList", workSeniorityList);
/* 124 */     model.addAttribute("degreeList", degreeList);
/* 125 */     ShopFrontHelper.setCommonData(request, model, web, 1);
/* 126 */     return web.getTplSys("member", MessageResolver.getMessage(request, "tpl.memberProfile", new Object[0]));
/*     */   }
/*     */ 
...
/*     */     }
/* 147 */     bean = this.manager.update(bean, groupId, userDegreeId, degreeId, incomeDescId, workSeniorityId, familyMembersId);
/* 148 */     log.info("ShopMember update infomation: {}", bean.getUsername());
...
/* 164 */     ShopMember member = MemberThread.get();
/*     */ 
/* 166 */     if (member == null) {
/* 167 */       return "redirect:../login.jspx";
/*     */     }
...
/* 172 */     return web.getTplSys("member", MessageResolver.getMessage(request, "tpl.memberPassword", new Object[0]));
/*     */   }
/*     */ 
...
/*     */ 
/* 241 */     if (member == null) {
/* 242 */       return "redirect:../login.jspx";
/*     */     }
/* 244 */     Long userId = member.getMember().getUser().getId();
/* 245 */     WebErrors errors = validatePassword(userId, email, newPwd, member.getEmail(), origPwd, request);
/* 246 */     if (errors.hasErrors()) {
/* 247 */       return FrontHelper.showError(errors, web, model, request);
/*     */     }
/* 249 */     this.userMng.updateUser(userId, newPwd, email);
/* 250 */     log.info("ShopMember update password: {}", member.getUsername());
/* 251 */     return FrontHelper.showSuccess("global.success", nextUrl, web, model, request);
/*     */   }
/*     */ 
...
/* 266 */     Long userId = member.getMember().getUser().getId();
/* 267 */     boolean pass = this.userMng.isPasswordValid(userId, origPwd);
/* 268 */     ResponseUtils.renderJson(response, pass ? "true" : "false");
/*     */   }
/*     */ 
...
/* 288 */     if ((!StringUtils.isBlank(newPwd)) && 
...
/*     */     }
/* 295 */     if (errors.ifNotEmail(email, "email", 100)) {
/* 296 */       return errors;
/*     */     }
