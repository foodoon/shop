...
/*     */ 
/*  34 */   @RequestMapping({"/gift/v_list.do"})
...
/*     */     }
/*  51 */     model.addAttribute("gift", this.manager.findById(id));
/*  52 */     return "gift/edit";
/*     */   }
/*     */ 
...
/*     */   {
/*  58 */     bean = this.manager.save(bean);
/*  59 */     log.info("save brand. id={}", bean.getId());
...
/*     */   {
/*  66 */     this.manager.updateByUpdater(gift);
/*  67 */     log.info("update brand. id={}.", gift.getId());
...
/*     */     }
/*  78 */     Gift[] beans = this.manager.deleteByIds(ids);
/*  79 */     for (Gift bean : beans) {
/*  80 */       log.info("delete brand. id={}", bean.getId());
/*     */     }
...
/* 108 */     errors.ifEmpty(ids, "ids");
/* 109 */     for (Long id : ids) {
/* 110 */       vldExist(id, errors);
/*     */     }
/* 112 */     return errors;
/*     */   }
/*     */ 
...
/* 118 */     if (errors.ifEmpty(wids, "wids")) {
/* 119 */       return errors;
/*     */     }
/* 121 */     if (errors.ifEmpty(priority, "priority")) {
/* 122 */       return errors;
/*     */     }
/* 124 */     if (wids.length != priority.length) {
/* 125 */       errors.addErrorString("wids length not equals priority length");
/* 126 */       return errors;
/*     */     }
/* 128 */     int i = 0; for (int len = wids.length; i < len; i++) {
/* 129 */       vldExist(wids[i], errors);
/* 130 */       if (priority[i] == null) {
/* 131 */         priority[i] = Integer.valueOf(0);
/*     */       }
/*     */     }
/* 134 */     return errors;
/*     */   }
...
/* 138 */     Gift entity = this.manager.findById(id);
/* 139 */     return errors.ifNotExist(entity, Brand.class, id);
/*     */   }
/*     */ }
