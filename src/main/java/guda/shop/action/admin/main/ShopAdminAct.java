...
/*     */ 
/*  45 */   @RequestMapping({"/admin/v_list.do"})
...
/*  46 */     model.addAttribute(pagination);
/*  47 */     return "admin/list"; }
/*     */ 
...
/*  59 */     WebErrors errors = validateEdit(id, request);
...
/*  65 */     model.addAttribute("admin", this.manager.findById(id));
/*  66 */     model.addAttribute("roleIds", this.manager.findById(id).getAdmin().getRoleIds());
...
/*  79 */     bean = this.manager.register(username, password, viewonlyAdmin, email, request.getRemoteAddr(), disabled.booleanValue(), web.getId(), bean);
...
/*     */     }
/*  93 */     bean = this.manager.update(bean, password, disabled, email, viewonlyAdmin);
/*  94 */     this.adminMng.updateRole(bean.getAdmin(), roleIds);
/*  95 */     log.info("update ShopAdmin id={}.", bean.getId());
...
/*     */   {
/* 127 */     if ((StringUtils.isBlank(email)) || (this.userMng.emailExist(email)))
/* 128 */       ResponseUtils.renderJson(response, "false");
/*     */     else {
/* 130 */       ResponseUtils.renderJson(response, "true");
/*     */     }
/* 132 */     return null;
/*     */   }
/*     */ 
...
/* 168 */     return errors.ifNotExist(entity, ShopAdmin.class, id);
/*     */   }
/*     */ 
...
/* 176 */       int i = 0; for (int len = arr.length; i < len; i++) {
