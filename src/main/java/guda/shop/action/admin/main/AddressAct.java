...
/*     */ 
/*  31 */   @RequestMapping({"/address/v_list.do"})
...
/*  34 */     model.addAttribute("parentId", parentId);
/*  35 */     return "address/list"; }
/*     */ 
/*     */   @RequestMapping({"/address/v_add.do"})
/*     */   public String add(Long parentId, ModelMap model)
/*     */   {
/*  40 */     model.addAttribute("parentId", parentId);
/*  41 */     return "address/add";
/*     */   }
/*     */ 
...
/*     */     }
/*  50 */     model.addAttribute("address", this.manager.findById(id));
/*  51 */     model.addAttribute("parentId", parentId);
/*  52 */     return "address/edit";
/*     */   }
/*     */ 
/*     */   @RequestMapping({"/address/o_save.do"})
/*     */   public String save(Long parentId, Address bean, HttpServletRequest request, ModelMap model) {
/*  57 */     WebErrors errors = validateSave(bean, request);
/*  58 */     if (errors.hasErrors()) {
/*  59 */       return errors.showErrorPage(model);
/*     */     }
/*  61 */     if (parentId != null) {
/*  62 */       Address address = this.manager.findById(parentId);
/*  63 */       bean.setParent(address);
/*     */     }
/*  65 */     bean = this.manager.save(bean);
/*  66 */     log.info("save Address id={}", bean.getId());
/*  67 */     model.addAttribute("parentId", parentId);
...
/*     */     }
/*  78 */     bean = this.manager.update(bean);
/*  79 */     log.info("update Address id={}.", bean.getId());
/*  80 */     return list(parentId, pageNo, request, model);
/*     */   }
/*     */ 
...
/*     */     }
/*  90 */     Address[] beans = this.manager.deleteByIds(ids);
/*  91 */     for (Address bean : beans) {
/*  92 */       log.info("delete Address id={}", bean.getId());
/*     */     }
/*  94 */     return list(parentId, pageNo, request, model);
/*     */   }
/*     */ 
...
/*     */   {
/* 100 */     WebErrors errors = validatePriority(wids, priority, request);
/* 101 */     if (errors.hasErrors()) {
/* 102 */       return errors.showErrorPage(model);
/*     */     }
/* 104 */     this.manager.updatePriority(wids, priority);
/* 105 */     model.addAttribute("message", "global.success");
/* 106 */     return list(parentId, pageNo, request, model);
/*     */   }
/*     */ 
...
/* 112 */     if (errors.ifEmpty(wids, "wids")) {
/* 113 */       return errors;
/*     */     }
/* 115 */     if (errors.ifEmpty(priority, "priority")) {
/* 116 */       return errors;
/*     */     }
/* 118 */     if (wids.length != priority.length) {
/* 119 */       errors.addErrorString("wids length not equals priority length");
/* 120 */       return errors;
/*     */     }
/* 122 */     int i = 0; for (int len = wids.length; i < len; i++) {
/* 123 */       vldExist(wids[i], errors);
/* 124 */       if (priority[i] == null) {
/* 125 */         priority[i] = Integer.valueOf(0);
/*     */       }
/*     */     }
/* 128 */     return errors;
/*     */   }
...
/* 132 */     Address entity = this.manager.findById(id);
/* 133 */     return errors.ifNotExist(entity, Address.class, id);
/*     */   }
/*     */ 
...
/* 145 */     Website web = SiteUtils.getWeb(request);
/* 146 */     if (vldExist(id, web.getId(), errors)) {
/* 147 */       return errors;
/*     */     }
/* 149 */     return errors;
/*     */   }
...
/* 153 */     WebErrors errors = WebErrors.create(request);
/* 154 */     Website web = SiteUtils.getWeb(request);
/* 155 */     if (vldExist(id, web.getId(), errors)) {
/* 156 */       return errors;
/*     */     }
/* 158 */     return errors;
/*     */   }
...
/* 163 */     Website web = SiteUtils.getWeb(request);
/* 164 */     errors.ifEmpty(ids, "ids");
/* 165 */     for (Long id : ids) {
/* 166 */       vldExist(id, web.getId(), errors);
/*     */     }
/* 168 */     return errors;
/*     */   }
/*     */ 
/*     */   private boolean vldExist(Long id, Long webId, WebErrors errors) {
/* 172 */     if (errors.ifNull(id, "id")) {
/* 173 */       return true;
/*     */     }
/* 175 */     Address entity = this.manager.findById(id);
/* 176 */     if (errors.ifNotExist(entity, Address.class, id)) {
/* 177 */       return true;
/*     */     }
/*     */ 
/* 183 */     return false;
