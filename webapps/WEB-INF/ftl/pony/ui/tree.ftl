...
<div id="${id}">
<div id="${id}-s" onmouseover="Pn.Tree.lineOver(this)" onmouseout="Pn.Tree.lineOut(this)"
...
<#--节点列-->
    ...
    <#if isCheckBox>
        <input type="checkbox" name="${checkBoxName}" <#rt/>
               <#if valueContent?seq_contains(node.id)>checked="true"</#if><#lt/>value="${node.id}" id="${id}-chk"
               vld="${_checkbox_vld}" class="pntree-checkbox" size="0" hidefocus="true"
               onclick="Pn.Tree.switchSelect(this,'${id}','${treeId}');"/><#t/>
    </#if>
<#--名称列-->
    <#if !isLeaf>
        <#if !isCheckBox>
