...
ajaxQueue.go = function () {
    ...
    if (!ajaxQueue.confirmFlag && a.type == "other") {
        cart2.blockUI()
    }
    jQuery.post(a.getUrl(), a.getParam(),
        function (b) {
            ...
            if (!redirectIf(b)) {
                if (!ajaxQueue.confirmFlag && a.type == "other") {
                    cart2.unblockUI(a.showMsg)
                }
                if (a.type != "check") {
                    refreshCart2PageContent(b)
                }
                ajaxQueue.go()
            } else {
                cart2.unblockUI("0");
...
var cart2 = {
    ...
    isBlocked: false,
    ajaxSelect: function () {
        optMan.mergeReq("select", null, "0", 1000);
    },
    ajaxDelete: function (b, a, c, d) {
        optMan.mergeReq("delete", null, null, c, b, a, d);
    },
    ajaxNum: function (a, d, c, b, e) {
        optMan.mergeReq("num", a, d, c, b, e);
    },
    ajaxLandingNum: function (b, e, d, c, f, a) {
        optMan.mergeReq("landingNum", b, e, d, c, f, a);
    },
    ajaxRefresh: function (a, c, b) {
        optMan.mergeReq("other", a, c, b);
    },
    ...
    unblockUI: function (a) {
        ...
        if (a && a == "0") {
            ...
            cart2.blockUITimer = setTimeout(function () {
                jQuery("#cart2BlockUI").unblock().hide();
            }, 1000);
        }
    }
...
optMan.mergeNum = function (a, c, f) {
    ...
    for (var b in this.map.entry) {
        ...
        if (d != null && d.stage == 1) {
            ...
            if (d.type == "num" && d.productId == c && d.merchantId == f) {
                d.url = a;
                d.clearTimeout();
                d.setTimeout();
                e = false
...
optMan.mergeReq = function (b, a, f, e, h, d, c) {
    if (ajaxQueue.confirmFlag && b != "check") {
        return;
    }
    if (this.map.size() <= 0) {
...
optMan.buildReq = function (b, a, g, f, h, e, d) {
    ...
    if (b == "delete") {
        ...
        c.addDeletePost = function (k, j, i) {
            switch (j) {
                case 0:
                    this.deleteQueue.push(k);
                    break;
                case 1:
                    this.deleteOverPromotionQueue.push(k);
                    break;
                case 2:
                    this.deleteWarningQueue.push(k);
                    break;
                case 3:
                    this.deleteGiftQueue.push(k);
                    break;
                case 4:
                    this.deleteErrorPromotionQueue.push(k);
                    break
            }
    ...
    c.getUrl = function () {
        ...
        if (this.type == "select") {
            return"/cart/ajax.do?action=select&rd=" + Math.random()
        }
        return this.url;
    };
    c.post = function () {
